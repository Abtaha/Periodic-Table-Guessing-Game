...
public class OEConfigureColor {
    public static void main(String argv[]) {
        OEInterface itf = new OEInterface();
        oedepict.OEConfigureColor(itf, "mycolor", "c", "Example of using OEConfigureColor", "blue");

        if (oechem.OECheckHelp(itf, argv, "OEConfigureColor")) {
            return;
        }

        if (!oechem.OEParseCommandLine(itf, argv, "OEConfigureColor")) {
            oechem.OEThrow.Fatal("Unable to interpret command line!");
        }

        OEColor color = oedepict.OEGetColor(itf, "mycolor");
