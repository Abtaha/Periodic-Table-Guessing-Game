...
public class AltlocfactGroupsDoc {

    static void PrintAltGroupInfo(OEMolBase mol) {
        if (! oechem.OEHasResidues(mol))
            oechem.OEPerceiveResidues(mol, OEPreserveResInfo.All);

        // create factory for mol
        ...
        for (OEAltGroup grp : alf.GetGroups()) {
            System.out.println("\t" + grp.GetLocationCount() + " locs"
                    + ":" + alf.GetLocationCodes(grp));
        }
    }
    public static void main(String argv[]) {
        if (argv.length != 1) {
            oechem.OEThrow.Usage("AltlocfactGroupsDoc <mol-infile>");
        }
