...
public class BindingSite1 { 
    private static final float _maxDistanceSquared = 8.0f * 8.0f;

    private static float getDistance (OEFloatArray acoords, OEFloatArray vcoords) {
        float d2 = (vcoords.getItem(0)-acoords.getItem(0)) * (vcoords.getItem(0)-acoords.getItem(0));
        d2 += (vcoords.getItem(1)-acoords.getItem(1)) * (vcoords.getItem(1)-acoords.getItem(1));
        d2 += (vcoords.getItem(2)-acoords.getItem(2)) * (vcoords.getItem(2)-acoords.getItem(2));
        return d2;
    }
    ...
    {
        if (args.length != 3) {
            oechem.OEThrow.Usage("BindingSite1 <protein> <ligand> <surface>");
        }
        ...
        OEFloatArray xyz  = new OEFloatArray(3);
        ...
        for (int i = 0;  i < surf.GetNumVertices(); i++) {
            ...
            for(OEAtomBase atom: lig.GetAtoms()) {
                lig.GetCoords(atom, xyz);
                float dist2 = getDistance(xyz, vert);
                if (dist2 < _maxDistanceSquared)
